...
- Bump `version` in `package.json` in the same change whenever files that gate the `release` job are touched (`src/`, `.github/workflows/ci.yml`, `package.json` itself): a merge of the image inputs publishes a Release only when the version moved, so a qualifying pull request without a bump silently releases nothing. Whether the bump is major, minor or patch follows the nature of the change (semver), decided before the final push that opens the pull request.
