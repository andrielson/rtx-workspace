...
beforeAll(async () => {
  ...
  await $`docker compose --file ${file} --project-name ${project} build --pull`.env(env)
}, 1_900_000)
...
describe('Dockerfile', () => {
  ...
  describe('copied tools', () => {
    test('go runs from the copied tree', async () => {
      ...
      expect(await execInWorkspace('go env GOROOT')).toBe('/home/ubuntu/.local/go')
    })

    test('rustup resolves the stable default toolchain', async () => {
      expect(await execInWorkspace('rustup show active-toolchain')).toMatch(/^stable-\S+ \(default\)$/)
    })

    test('cargo runs', async () => {
      expect(await execInWorkspace('cargo --version')).toMatch(/cargo \d+\.\d+\.\d+/)
    })

    test('rustc runs', async () => {
      expect(await execInWorkspace('rustc --version')).toMatch(/rustc \d+\.\d+\.\d+/)
    })
    ...
    test('bunx runs', async () => {
      expect(await execInWorkspace('bunx --version')).toMatch(/^\d+\.\d+\.\d+$/)
    })

    test('docker CLI runs', async () => {
      expect(await execInWorkspace('docker --version')).toMatch(/Docker version \d+\.\d+\.\d+/)
    })

    test('docker compose plugin is discovered', async () => {
      expect(await execInWorkspace('docker compose version')).toMatch(/Docker Compose version v\d+/)
    })
    ...
    test('shellcheck reports its version', async () => {
      ...
      expect(await execInWorkspace('shellcheck --version')).toMatch(/^version: \d+\.\d+(\.\d+)?$/m)
    })

    test('shfmt reports its version', async () => {
      expect(await execInWorkspace('shfmt --version')).toMatch(/^v\d+\.\d+\.\d+$/)
    })

    test.each(['shellcheck', 'shfmt'])('%s resolves from the copied location', async (command) => {
      expect(await execInWorkspace(`command -v ${command}`)).toBe(`/home/ubuntu/.local/bin/${command}`)
    })
  })
  ...
  describe('copied files and permissions', () => {
    ...
    test.each(['/usr/local/bin/docker-entrypoint', '/etc/profile.d/01-home-bash-env.sh'])(
      '%s is executable',
      async (path) => {
    ...
    test.each(['/run/sshd', '/home/ubuntu/.docker/cli-plugins'])('%s is a directory', async (path) => {
      await execInWorkspace(`test -d ${path}`)
    })

    test('the sudoers drop-in is root-owned mode 0440', async () => {
      expect(await execInWorkspace('stat --format=%a /etc/sudoers.d/ubuntu')).toBe('440')
      expect(await execInWorkspace('stat --format=%U:%G /etc/sudoers.d/ubuntu')).toBe('root:root')
    })

    test('/home/ubuntu is owned by ubuntu', async () => {
      expect(await execInWorkspace('stat --format=%U:%G /home/ubuntu')).toBe('ubuntu:ubuntu')
    })
    ...
    test('nothing under /home/ubuntu is group- or other-writable', async () => {
      expect(await execInWorkspace('find /home/ubuntu ! -type l -perm /go+w -print -quit')).toBe('')
    })

    test('the docker CLI plugins are symlinked onto PATH', async () => {
      expect(await execInWorkspace('find /home/ubuntu/.local/bin -maxdepth 1 -type l -print -quit')).not.toBe('')
    })
  })

  describe('privileges', () => {
    test('ubuntu has passwordless sudo', async () => {
      expect(await execInWorkspace('sudo --non-interactive id --user')).toBe('0')
    })
